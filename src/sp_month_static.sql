...
-- 导出  过程 fh_xydd.sp_month_static 结构
DROP PROCEDURE IF EXISTS `sp_month_static`;
DELIMITER //
CREATE DEFINER=`fh_xyddadmin`@`%` PROCEDURE `sp_month_static`(`localId` varchar(36) charset gbk,  `yearMo` varchar(36) charset gbk,paraCode varchar(20))
BEGIN
...
set monstr = concat('update temp_month m, (select ',paraCode,' maxv,date_time from t_static_pipe where local_id = \'',localId,'\' and date_format(date_time,\'%Y-%m\') = date_format(\'',yearMo,'\',\'%Y-%m\') and ',paraCode,' is not null  order by maxv desc limit 1)n set m.colvalue = round(n.maxv,1) where m.montime = \'最大值\'');
set @myquery = monstr;
prepare stmt from @myquery;
execute stmt;

set monstr = concat('update temp_month m, (select ',paraCode,' maxv,date_time from t_static_pipe where local_id = \'',localId,'\' and date_format(date_time,\'%Y-%m\') = date_format(\'',yearMo,'\',\'%Y-%m\') and ',paraCode,' is not null  order by maxv desc limit 1)n set m.colvalue = n.date_time where m.montime = \'最大时间\'');
set @myquery = monstr;
prepare stmt from @myquery;
execute stmt;

set monstr = concat('update temp_month m, (select ',paraCode,' maxv,date_time from t_static_pipe where local_id = \'',localId,'\' and date_format(date_time,\'%Y-%m\') = date_format(\'',yearMo,'\',\'%Y-%m\') and ',paraCode,' is not null order by maxv asc limit 1)n set m.colvalue = round(n.maxv,1) where m.montime = \'最小值\'');
set @myquery = monstr;
prepare stmt from @myquery;
execute stmt;

set monstr = concat('update temp_month m, (select ',paraCode,' maxv,date_time from t_static_pipe where local_id = \'',localId,'\' and date_format(date_time,\'%Y-%m\') = date_format(\'',yearMo,'\',\'%Y-%m\') and ',paraCode,' is not null order by maxv asc limit 1)n set m.colvalue = n.date_time where m.montime = \'最小时间\'');
set @myquery = monstr;
prepare stmt from @myquery;
execute stmt;

set monstr = concat('update temp_month m, (select sum(',paraCode,') maxv,date_time from t_static_pipe where local_id = \'',localId,'\' and date_format(date_time,\'%Y-%m\') = date_format(\'',yearMo,'\',\'%Y-%m\') order by maxv asc limit 1)n set m.colvalue = round(n.maxv,1) where m.montime = \'月合计\'');
set @myquery = monstr;
prepare stmt from @myquery;
execute stmt;

set monstr = concat('update temp_month m, (select avg(',paraCode,') maxv,date_time from t_static_pipe where local_id = \'',localId,'\' and date_format(date_time,\'%Y-%m\') = date_format(\'',yearMo,'\',\'%Y-%m\') order by maxv asc limit 1)n set m.colvalue = round(n.maxv,1) where m.montime = \'日平均\'');
set @myquery = monstr;
prepare stmt from @myquery;
execute stmt;
########################################
...
if paraCode = 'g10_day' then

   alter table temp_month add weather varchar(36);
   update temp_month m,(select date,weather from t_weather where date >= yearMo and  date <= date_add(yearMo,interval 1 month))n set m.weather = n.weather where m.montime = n.date;
   select montime,colvalue,weather from temp_month;
   else 
